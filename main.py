...
if __name__ == "__main__":
    ...
    parser.add_argument('--initial-recovery-checkpoint', dest='initial_recovery_checkpoint', type=str, help='LoRA recovery checkpoint used to initialize staged prompt recovery')
    ...
    parser.add_argument('--prompt-components', dest='prompt_components', type=str, default='kv', help='Comma-separated prompt components for timm_pruned_prompt: vpt,kv')
    parser.add_argument('--prompt-mode', dest='prompt_mode', choices=['shallow', 'deep'], default='shallow', help='Visual prompt placement for VPT-based recovery')
    ...
    parser.add_argument('--num-kv-prompt-tokens', dest='num_kv_prompt_tokens', type=int, default=5, help='Number of key-value prompt tokens per attention layer')
    parser.add_argument('--kv-prompt-tokens-per-layer', dest='kv_prompt_tokens_per_layer', type=str, default=None, help='Comma-separated KV prompt token counts, one per transformer block')
    parser.add_argument('--share-kv-prompt', dest='share_kv_prompt', action=argparse.BooleanOptionalAction, default=True, help='Share one prompt parameter between keys and values')
